...
REQUIRED_APIS="compute.googleapis.com iap.googleapis.com secretmanager.googleapis.com iam.googleapis.com cloudresourcemanager.googleapis.com logging.googleapis.com monitoring.googleapis.com"
...
if [[ "${SM_READY}" == "true" ]]; then
  ...
  OPENCLAW_SECRETS=(
    ...
    # ── GitHub integration (empty = no initial version) ───────────────────
    "GITHUB_TOKEN||github"
...
echo ""
echo "  To run multiple agents (each with its own Discord/Telegram"
echo "  bot), add secrets with the naming pattern:"
echo ""
echo "    DISCORD_BOT_TOKEN_AGENTNAME"
echo "    TELEGRAM_BOT_TOKEN_AGENTNAME"
echo "    SLACK_BOT_TOKEN_AGENTNAME"
echo ""
echo "  Then SSH into the VM and register each account:"
echo ""
echo "    sudo -u openclaw openclaw channels add \\"
echo "      --channel discord --account agentname --token 'token'"
echo ""
echo "  Configure agent routing:"
echo ""
echo "    sudo -u openclaw openclaw config set bindings '[..."
echo "      {\"agentId\":\"agentname\",\"match\":{\"channel\":\"discord\",\"accountId\":\"agentname\"}}"
echo "    ...]' --json"
echo ""
echo "  Then restart:  sudo systemctl restart openclaw-gateway"
echo ""
echo "  ── GitHub Integration ───────────────────────────────────"
echo ""
echo "  To enable agents to work with GitHub repositories,"
echo "  add versions to these pre-created secrets:"
echo ""
echo "    GITHUB_TOKEN     — personal access token or fine-grained token"
